...
app.post('/send-order-alert', async (req, res) => {
  try {
    ...
    const {
      phone, telephone,
      product, quantity, supplier, threshold,
      orderId, orderID, orderld,
      message // On récupère le message pré-construit
    } = req.body;

    const finalPhone = phone || telephone;
    const finalOrderId = orderId || orderID || orderld;

    // 1. Vérification minimale : on a besoin d'un numéro
    if (!finalPhone) {
      return res.status(400).json({ status: 'error', message: 'Numéro manquant' });
    }

    // 2. Si AUCUN message n'est fourni, on exige les détails de stock pour construire l'alerte
    if (!message && (!product || quantity === undefined || !supplier || threshold === undefined)) {
      return res.status(400).json({ status: 'error', message: 'Données produit incomplètes et aucun message fourni' });
    }
    ...
    // 4. Si une commande est associée, on la garde en mémoire pour la réponse
    if (finalOrderId) {
    ...
    // 5. Choix du contenu : soit le message reçu, soit l'alerte reconstruite
    const finalMessage = message || `🚨 *ALERTE STOCK FAIBLE*\n\n📦 *Produit :* ${product}\n📊 *Qté :* ${quantity}\n⚠️ *Seuil :* ${threshold}\n🏪 *Fournisseur :* ${supplier}`;

    console.log('📤 Envoi du message WhatsApp à :', whatsappId);
    await sock.sendMessage(whatsappId, { text: finalMessage });

    return res.json({ status: 'success', orderId: finalOrderId || 'n/a' });
  } catch (error) {
